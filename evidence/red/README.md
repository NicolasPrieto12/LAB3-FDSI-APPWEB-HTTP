...
- [nmap_port80.txt](nmap_port80.txt) — puerto 80 abierto, `nginx 1.28.3 (Ubuntu)` expuesto (evidencia R2)
- [curl_home.txt](curl_home.txt) — `200 OK` sin autenticación (evidencia R3)
- [curl_headers.txt](curl_headers.txt) — `404` en `/public-inventory.txt` (el sitio solo tiene `index.html`)
